...
class LastPage: UIViewController {
    ...
    let emoji: EmojiController = EmojiController()
     ...
     func nextButtonTouched(_ sender: UIButton) {
    ...
    func backButtonTouched(_ sender:UIButton) {
        if (index - 1 >= 0){
            index = index - 1
            print(index)
            self.getImage(index);
    ...
    func initNextButton(){
        next.addTarget(self, action: #selector(LastPage.nextButtonTouched(_:)), for: UIControlEvents.touchUpInside)
    ...
    func initBackButton(){
        ...
        back.addTarget(self, action: #selector(LastPage.backButtonTouched(_:)), for: UIControlEvents.touchUpInside)
    ...
    override func viewDidLoad() {
        ...
        self.done.backgroundColor = UIColor.clear
        self.initNextButton()
        self.initBackButton()
        self.gifView.frame = CGRect(x: 40.0, y: 200.0, width: 300.0, height: 200.0)
